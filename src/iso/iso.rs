// isobemak/src/iso.rs

use crate::utils::{pad_to_lba, update_4byte_fields, ISO_SECTOR_SIZE};
use crate::iso::dir_record::IsoDirEntry;
use crate::iso::volume_descriptor::*;
use crate::iso::boot_catalog::*;
use std::fs::File;
use std::io::{self, copy, Write, Seek};
use std::path::Path;

/// Creates an ISO image from a bootable image file.
pub fn create_iso_from_img(iso_path: &Path, boot_img_path: &Path) -> io::Result<()> {
    println!("create_iso_from_img: Starting ISO creation");

    let boot_img_size = std::fs::metadata(boot_img_path)?.len();
    let boot_img_sectors = ((boot_img_size + 511) / 512) as u16;

    let mut iso = File::create(iso_path)?;

    let root_entry = IsoDirEntry { lba: 20, size: ISO_SECTOR_SIZE as u32, flags: 0x02, name: "." };
    write_primary_volume_descriptor(&mut iso, 0, &root_entry)?;
    write_boot_record_volume_descriptor(&mut iso, LBA_BOOT_CATALOG)?;
    write_volume_descriptor_terminator(&mut iso)?;

    write_boot_catalog(&mut iso, 23, boot_img_sectors)?;

    // ISO9660 directories simplified
    pad_to_lba(&mut iso, 20)?;
    let root_dir = vec![
        IsoDirEntry{lba:20,size:ISO_SECTOR_SIZE as u32,flags:0x02,name:"."}.to_bytes(),
        IsoDirEntry{lba:20,size:ISO_SECTOR_SIZE as u32,flags:0x02,name:".."}.to_bytes(),
        IsoDirEntry{lba:21,size:ISO_SECTOR_SIZE as u32,flags:0x02,name:"EFI"}.to_bytes(),
        IsoDirEntry{lba:LBA_BOOT_CATALOG,size:ISO_SECTOR_SIZE as u32,flags:0x00,name:"BOOT.CATALOG"}.to_bytes(),
    ].concat();
    let mut root_dir_content = root_dir;
    root_dir_content.resize(ISO_SECTOR_SIZE,0);
    iso.write_all(&root_dir_content)?;

    pad_to_lba(&mut iso, 21)?;
    let efi_dir = vec![
        IsoDirEntry{lba:21,size:ISO_SECTOR_SIZE as u32,flags:0x02,name:"."}.to_bytes(),
        IsoDirEntry{lba:20,size:ISO_SECTOR_SIZE as u32,flags:0x02,name:".."}.to_bytes(),
        IsoDirEntry{lba:22,size:ISO_SECTOR_SIZE as u32,flags:0x02,name:"BOOT"}.to_bytes(),
    ].concat();
    let mut efi_dir_content = efi_dir;
    efi_dir_content.resize(ISO_SECTOR_SIZE,0);
    iso.write_all(&efi_dir_content)?;

    pad_to_lba(&mut iso, 22)?;
    let boot_dir = vec![
        IsoDirEntry{lba:22,size:ISO_SECTOR_SIZE as u32,flags:0x02,name:"."}.to_bytes(),
        IsoDirEntry{lba:21,size:ISO_SECTOR_SIZE as u32,flags:0x02,name:".."}.to_bytes(),
        IsoDirEntry{lba:23,size:boot_img_size as u32,flags:0x00,name:"BOOTX64.EFI"}.to_bytes(),
    ].concat();
    let mut boot_dir_content = boot_dir;
    boot_dir_content.resize(ISO_SECTOR_SIZE,0);
    iso.write_all(&boot_dir_content)?;

    pad_to_lba(&mut iso, 23)?;
    let mut boot_img_file = File::open(boot_img_path)?;
    copy(&mut boot_img_file, &mut iso)?;

    let final_pos = iso.stream_position()?;
    let total_sectors = (final_pos + (ISO_SECTOR_SIZE as u64 - 1)) / ISO_SECTOR_SIZE as u64;
    update_4byte_fields(&mut iso, 16, PVD_TOTAL_SECTORS_OFFSET, PVD_TOTAL_SECTORS_OFFSET + 4, total_sectors as u32)?;
    iso.set_len(total_sectors * ISO_SECTOR_SIZE as u64)?;

    println!("create_iso_from_img: ISO created with {} sectors", total_sectors);
    Ok(())
}
