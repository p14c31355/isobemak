...
fn main() -> std::io::Result<()> {
    ...
    let mut f = OpenOptions::new().read(true).write(true).create(true).truncate(true).open(&img)?;
    ...
    assert_eq!(v6b, [0xEB, 0x58, 0x90], "Sector 6 corrupted after data write");
    ...
    assert_eq!(v6c, [0xEB, 0x58, 0x90], "Sector 6 corrupted after FAT write");
    ...
    println!("Final sector 6: {:02x} {:02x} {:02x} (expected eb 58 90)", v6d[0], v6d[1], v6d[2]);

    // Also verify sector 0
    let mut v0d = [0u8; 3];
    f2.seek(SeekFrom::Start(0))?;
    f2.read_exact(&mut v0d)?;
    println!("Final sector 0: {:02x} {:02x} {:02x} (expected eb 58 90)", v0d[0], v0d[1], v0d[2]);
    ...
    for s in 0..8u64 {
        ...
        println!("Sector {}: {}", s, buf.iter().map(|b| format!("{:02x}", b)).collect::<Vec<_>>().join(" "));
    }
