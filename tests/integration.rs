...
fn test_create_isohybrid_uefi_iso() -> io::Result<()> {
    ...
    let (fat_image_path, _temp_fat_file_holder, _iso_file) =
    ...
    let fat_img_metadata = std::fs::metadata(&fat_image_path)?;
    let expected_esp_sectors =
        (fat_img_metadata.len() as u32).div_ceil(isobemak::utils::ISO_SECTOR_SIZE as u32) as u16;
